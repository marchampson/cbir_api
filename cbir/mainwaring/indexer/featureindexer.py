...
class FeatureIndexer(BaseIndexer):
    def __init__(self, dbPath, estNumImages=500, maxBufferSize=50000, dbResizeFactor=2,
        ...
        # open the HDF5 database for writing and initialising the datasets within
        # the group
        ...
        # initialise the image IDs buffer, index buffer and the keypoints +
        # features buffer
        self.imageIDBuffer = []
        self.indexBuffer = []
        self.featuresBuffer = None

        # initialise the total number of features in the buffer along with the
        # indexes dictionary
    ...
    def _createDatasets(self):
        ...
        # and use this number to determine the approx number of features for
        # the entire dataset
        ...
        self.imageIDDB = self.db.create_dataset("image_ids", (self.estNumImages,), maxshape=(None,), dtype=h5py.special_dtype(vlen=unicode))
    ...
    def finish(self):
        # if the databases have not been initialised, then the original
