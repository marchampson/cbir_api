...
class BaseIndexer(object):
    def __init__(self, dbPath, estNumImages=500, maxBufferSize=50000, dbResizeFactor=2,
                 verbose=True):
        # store the database path, estimated number of imqages in the dataset, max
        # buffer size, the resize factor of the db and the verbosity setting
        self.dbPath = dbPath
        ...
        # initialie the indexes dictionary
        self.idxs = {}
    ...
    def _writeBuffer(self, dataset, datasetName, buf, idxName, sparse=False):
        # if the buffer is a list, then compute the endinf index base on
        # the list length
        if type(buf) is list:
        ...
        # if this is a sparse SciPy matrix, then convert the sparse matrix to a
        # dense one so it can be written to file
        # HDF5 can only store dense arrays
        if sparse:
            buf = buf.toarray()
    ...
    def _resizeDataset(self, dataset, dbName, baseSize=0, finished=0):
        ...
        # so, make the new size the current index, finished will be the new number of rows
        if finished > 0:
            newSize = finished
    ...
    def featureStack(array, accum=None, stackMethod=np.vstack):
        # if the accumulated array is None, initialise it
        if accum is None:
            accum = array
